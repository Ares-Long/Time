...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top): # n_top: c*ln(L_q)
        ...
        index_sample = torch.randint(L_K, (L_Q, sample_k))  # real U = U_part(factor*ln(L_k))*L_q   96*25 生成随机0-95的整数
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        queries = self.query_projection(queries).view(B, L, H, -1)  # 重新定义矩阵形状，-1代表动态调整这个维度上的个数
